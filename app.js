...
async function handleUpload(file) {
  ...
  await retryUpload(() =>
    sb.storage.from("thumbs").upload(name + ".jpg", thumbBlob, {
      contentType: "image/jpeg"
    })
  );

  await retryUpload(() =>
    sb.storage.from("videos").upload(name + ".mp4", file, {
  ...
  await retryUpload(() =>
    sb.from("videos").insert([{ video_url: videoUrl, thumb_url: thumbUrl }])
  );
